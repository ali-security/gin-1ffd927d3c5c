...
// TestSetTrustedProxies asserts that narrowing the trusted proxy set takes
// effect on the plain ServeHTTP path, without any call to Engine.Run, and that
// an unparsable entry is reported back to the caller.
func TestSetTrustedProxies(t *testing.T) {
	...
	// Every proxy is trusted by default, so the header is honoured as it was
	// before the trusted proxies feature existed.
	requestWith("6.6.6.6:1234", "1.1.1.1")
	assert.Equal(t, "1.1.1.1", clientIP)

	// Narrowing the trusted set takes effect immediately: the peer is no
	// longer a trusted proxy, so its forged header is ignored.
	assert.NoError(t, router.SetTrustedProxies([]string{"10.0.0.1"}))
	requestWith("6.6.6.6:1234", "1.1.1.1")
	assert.Equal(t, "6.6.6.6", clientIP)

	// Turning the feature off makes ClientIP return the remote address even
	// for a formerly trusted proxy.
	assert.NoError(t, router.SetTrustedProxies(nil))
	requestWith("10.0.0.1:1234", "1.1.1.1")
	assert.Equal(t, "10.0.0.1", clientIP)

	// An unparsable origin is reported, and no proxy ends up trusted.
	assert.Error(t, router.SetTrustedProxies([]string{"not-a-cidr"}))
	requestWith("10.0.0.1:1234", "1.1.1.1")
	assert.Equal(t, "10.0.0.1", clientIP)
}
